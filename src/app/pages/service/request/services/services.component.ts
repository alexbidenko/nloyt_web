...
export class ServicesComponent implements OnInit {
  ...
  addServiceToCheck(serviceId: number) {
    if (this.checkedServices.indexOf(serviceId) > -1) {
      this.checkedServices.splice(this.checkedServices.indexOf(serviceId), 1);
    } else {
  ...
  checkAllServices() {
    if (this.checkedServices.length === this.requestService.device.order.services.length) {
      this.checkedServices = [];
    } else {
      this.checkedServices = this.requestService.device.order.services.map(service => service.id);
    }
  }

  getTotalDuration(): number {
     return this.requestService.device.order.services.reduce((a, b) => a + b.duration, 0);
  }

  getTotalPrice(): number {
    return this.requestService.device.order.services.reduce((a, b) => a + b.price, 0);
  }
