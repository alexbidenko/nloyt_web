...
import {Device, Order, OrderFile, Services} from '../../../models/interfaces';
...
export class OrdersComponent implements OnInit, OnChanges {
  ...
  activeFilters = localStorage.getItem('orders.activeFilters') ?
    JSON.parse(localStorage.getItem('orders.activeFilters')) : {
    acceptNow: false,
    connection: false,
    rejectedByWS: false,
    cancelledByC: false,
  ...
  readingOrders = localStorage.getItem('readingOrders') ?
    JSON.parse(localStorage.getItem('readingOrders')) : [];
  ...
  changeFilter(key: string) {
    ...
    localStorage.setItem('orders.activeFilters', JSON.stringify(this.activeFilters));
    this.onWindowScroll();
  }
  ...
  addReadOrder(orderId: number) {
      this.readingOrders.push(orderId);
      localStorage.setItem('readingOrders', JSON.stringify(this.readingOrders));
  }

  isUnread(orderId: number): boolean {
    return this.readingOrders.indexOf(orderId) === -1;
  }

  getReduce(services: Services[], key: string): number {
      return services.reduce((a, b) => a + b[key], 0);
  }
}
